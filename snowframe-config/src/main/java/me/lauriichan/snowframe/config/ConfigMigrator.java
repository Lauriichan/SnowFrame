...
public final class ConfigMigrator {
    ...
    private final Object2ObjectArrayMap<Class<? extends IConfig>, Migration> migrations = new Object2ObjectArrayMap<>();

    public ConfigMigrator(final SnowFrame<?> snowFrame) {
        Object2ObjectArrayMap<Class<? extends IConfig>, ObjectArrayList<ConfigMigrationExtension<?>>> tmpMigrations = new Object2ObjectArrayMap<>();
        snowFrame.extension(ConfigMigrationExtension.class, true).callInstances(extension -> {
            Class<?> target = extension.targetType();
            if (target == null) {
                snowFrame.logger().warning("Couldn't register migration as it doesn't define a target: {0}", extension.getClass().getName());
                return;
            }
            if (extension.minVersion() < 0 || extension.minVersion() >= extension.targetVersion()) {
                snowFrame.logger().warning(
            ...
            Class<? extends IConfig> configTarget = target.asSubclass(IConfig.class);
    ...
    public int getTargetVersion(Class<? extends IConfig> extension) {
        Migration migration = migrations.get(extension);
        return migration == null ? 0 : migration.targetVersion();
    }

    public boolean needsMigration(Class<? extends IConfig> extension, int version) {
        Migration migration = migrations.get(extension);
        return migration != null && version < migration.targetVersion();
    }

    public <T extends IConfig> int migrate(ISimpleLogger logger, int version, Configuration configuration, T extension)
